...
本报告针对记忆库工具和向量数据库工具之间的端口配置进行了全面检查，确保按照要求将前者端口统一为8001，后者端口统一为8100。
...
**向量数据库工具 (端口 8100)：**
- ✅ `knowledge_base_service.py` - 向量数据库HTTP服务（根目录版本为主）
- ✅ `context_aggregator_mcp.py` - 向量数据库上下文聚合器
- ✅ `memory_processor.py` - 传统记忆处理器
- ⚠ `mcp-calculator/vector_db.py` - 计算器向量数据库工具（已归档，非权威）
- ⚠ `mcp-calculator/knowledge_base_service.py` - 计算器向量数据库服务（已归档，非权威）
...
**工具文件检查：**
- ✅ 4/4 MCP工具文件存在且正确配置（归档项不计入权威清单）
- ✅ 所有工具都正确导入 FastMCP
- ✅ 所有工具都定义了相应的 @mcp.tool() 函数
- ✅ 端口配置符合规范

**HTTP服务检查：**
- ✅ 2/2 HTTP服务文件存在且正确配置（归档项不计入权威清单）
- ✅ 所有服务都正确使用 FastAPI
- ✅ 所有服务都定义了相应的路由
- ✅ 端口配置符合规范
...
2. `knowledge-base` - 向量数据库MCP服务 (依赖端口8100)
3. `context-aggregator` - 向量数据库上下文聚合 (依赖端口8100)
4. `embedding-context-aggregator` - 记忆库上下文聚合 (依赖端口8001)

**HTTP服务：**
1. `knowledge-base-http` - 向量数据库HTTP服务 (端口8100)
...
> 说明：`mcp-calculator/` 下同名实现已归档，供历史参考或兼容使用；生产/主线以根目录版本为权威来源。
...
**报告生成时间：** 2025年1月17日  （本页于后续清理中补充了“归档”说明）  
**检查范围：** 完整项目代码库  
**检查状态：** ✅ 全部通过
