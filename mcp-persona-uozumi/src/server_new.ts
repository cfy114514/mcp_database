...
// 新增：引入"卡菈克"文件的绝对路径（configs目录）
const KARLACH_PERSONA_PATH = path.resolve(__dirname, "..", "..", "configs", "personas", "karlach", "persona.md");
const KARLACH_WORLD_BOOK_PATH = path.resolve(__dirname, "..", "..", "configs", "personas", "karlach", "karlach_worldbook.zh.json");
const KARLACH_LEVELS_PATH = path.resolve(__dirname, "..", "..", "configs", "personas", "karlach", "levels.v1.json");
const KARLACH_BUCKETS_PATH = path.resolve(__dirname, "..", "..", "configs", "personas", "karlach", "buckets.v1.json");
const KARLACH_TEMPLATES_PATH = path.resolve(__dirname, "..", "..", "configs", "personas", "karlach", "freeplay_templates.v1.json");
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      // --- 新增：Karlach 工具 ---
      {
        name: "get_karlach_persona",
        description: "获取 Karlach 角色的完整人设内容",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      {
        name: "get_karlach_system_prompt",
        description: "生成 Karlach 角色的系统提示（包含安全指南和人设）",
        inputSchema: {
          type: "object",
          properties: {
            user: { type: "string", description: "当前用户名称（替换 {{user}}）", default: "用户" },
            char: { type: "string", description: "角色名（替换 {{char}}）", default: "卡菈克" }
          },
          required: []
        }
      },
      {
        name: "get_karlach_safety_guidelines",
        description: "获取安全使用指南（卡菈克）",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      {
        name: "list_karlach_worldbook_entries",
        description: "列出 Karlach 世界书条目（基本信息）",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      {
        name: "get_karlach_worldbook_entry",
        description: "获取 Karlach 指定世界书条目（含分块）",
        inputSchema: { type: "object", properties: { id: { type: "string" } }, required: ["id"] }
      },
      {
        name: "search_karlach_worldbook",
        description: "在 Karlach 世界书中按关键词检索（字符串匹配）",
        inputSchema: { type: "object", properties: { query: { type: "string" }, top_k: { type: "number", default: 5 } }, required: ["query"] }
      },
      {
        name: "get_karlach_levels",
        description: "获取 Karlach 关卡配置",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      {
        name: "get_karlach_buckets",
        description: "获取 Karlach 语气桶配置",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      {
        name: "get_karlach_templates",
        description: "获取 Karlach 自由发挥模板",
        inputSchema: { type: "object", properties: {}, required: [] }
      },
      // --- Uozumi 原有世界书工具 ---
      {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "search_luoluo_worldbook": {
      try {
        ...
        for (const e of wb.entries || []) {
          ...
          if (keywords.includes(query)) entryScore += 2;
          if (title.includes(query)) entryScore += 1;
          ...
          if (Array.isArray(e.chunks)) {
            for (const c of e.chunks) {
              ...
              if (content.includes(query)) {
                results.push({ score: 3, entryId: e.id, chunkId: c.id, text: c.content });
              }
            }
          }
        }
    ...
    // --- 新增：Karlach ---
    case "get_karlach_persona":
      try {
        const persona = readAbs(KARLACH_PERSONA_PATH);
        return { content: [{ type: "text", text: `# Karlach 角色人设\n\n${persona}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 人设 - ${(error as Error).message}` }], isError: true };
      }

    case "get_karlach_system_prompt":
      try {
        const safety = read(SAFETY_PATH);
        let persona = readAbs(KARLACH_PERSONA_PATH);
        const user = String(args?.user || "用户");
        const char = String(args?.char || "卡菈克");
        persona = persona.replaceAll("{{user}}", user).replaceAll("{{char}}", char);
        const systemPrompt = [safety.trim(), "", persona.trim()].join("\n\n");
        return { content: [{ type: "text", text: `# Karlach 系统提示\n\n${systemPrompt}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法生成 Karlach 系统提示 - ${(error as Error).message}` }], isError: true };
      }

    case "get_karlach_safety_guidelines":
      try {
        const safety = read(SAFETY_PATH);
        return { content: [{ type: "text", text: `# Karlach 安全使用指南\n\n${safety}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 安全指南 - ${(error as Error).message}` }], isError: true };
      }

    case "list_karlach_worldbook_entries": {
      try {
        const raw = readAbs(KARLACH_WORLD_BOOK_PATH);
        const wb = JSON.parse(raw) as { entries: any[] };
        const summary = (wb.entries || []).map((e: any) => ({ 
          id: e.id, 
          keywords: e.keywords, 
        ...
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 世界书 - ${(e as Error).message}` }], isError: true };
      }
    }

    case "get_karlach_worldbook_entry": {
      try {
        const raw = readAbs(KARLACH_WORLD_BOOK_PATH);
        const wb = JSON.parse(raw) as { entries: any[] };
        const id = String(args?.id || "");
        const found = (wb.entries || []).find((e: any) => e.id === id);
        if (!found) return { content: [{ type: "text", text: `未找到 Karlach 条目：${id}` }], isError: true };
        return { content: [{ type: "text", text: JSON.stringify(found, null, 2) }] };
      } catch (e) {
        return { content: [{ type: "text", text: `错误：读取 Karlach 条目失败 - ${(e as Error).message}` }], isError: true };
      }
    }

    case "search_karlach_worldbook": {
      try {
        const raw = readAbs(KARLACH_WORLD_BOOK_PATH);
        const wb = JSON.parse(raw) as { entries: any[] };
        const query = String(args?.query || "").toLowerCase();
        ...
        return { content: [{ type: "text", text: `错误：检索 Karlach 世界书失败 - ${(e as Error).message}` }], isError: true };
      }
    }

    case "get_karlach_levels":
      try {
        const levels = readAbs(KARLACH_LEVELS_PATH);
        return { content: [{ type: "text", text: `# Karlach 关卡配置\n\n${levels}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 关卡配置 - ${(error as Error).message}` }], isError: true };
      }

    case "get_karlach_buckets":
      try {
        const buckets = readAbs(KARLACH_BUCKETS_PATH);
        return { content: [{ type: "text", text: `# Karlach 语气桶配置\n\n${buckets}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 语气桶配置 - ${(error as Error).message}` }], isError: true };
      }

    case "get_karlach_templates":
      try {
        const templates = readAbs(KARLACH_TEMPLATES_PATH);
        return { content: [{ type: "text", text: `# Karlach 自由发挥模板\n\n${templates}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `错误：无法读取 Karlach 模板配置 - ${(error as Error).message}` }], isError: true };
      }

    // --- Uozumi 原有世界书 ---
    case "list_worldbook_entries": {
      const wb = readWorldbook();
      if (!wb) return { content: [{ type: "text", text: "错误：无法读取世界书" }], isError: true };
    ...
    case "get_worldbook_entry": {
      ...
      const id = String(args?.id || "");
      const found = (wb.entries || []).find((e: any) => e.id === id);
      if (!found) return { content: [{ type: "text", text: `未找到条目：${id}` }], isError: true };
      return { content: [{ type: "text", text: JSON.stringify(found, null, 2) }] };
    }
